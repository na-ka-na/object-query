...
sql="SELECT all_employees.id, all_employees.name FROM ('argv[1]', 'Example1.Company') WHERE all_employees.id IS NOT NULL AND all_employees.name='abc' AND all_employees.active='true' AND financial.quarterly_profits>0"
